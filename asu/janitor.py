...
def update_branch(branch):
    ...
    for version in branch["versions"]:
        ...
        with Pool() as pool:
            # TODO: ugly
            version_path = current_app.config["JSON_PATH"] / branch["path"].format(
                version=version
            )
            ...
            packages_symlink = version_path / "packages"
            ...
            overview = {"branch": branch["name"], "release": version, "profiles": []}

            for profile_file in (version_path / "targets").rglob("**/*.json"):
            ...
            (version_path / "overview.json").write_text(
